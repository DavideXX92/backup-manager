...
namespace PDSclient
{
    class HandlePacketsSecure : HandlePackets
    {
        ...
        public HandlePacketsSecure(string serverIP, int port)
        {
            connectToTLSserver(serverIP, port);
        ...
        private void connectToServer(string serverIP, int port)
        {
            ...
            try
            {
                ...
                if (client.Connected)
                {
                    this.netStream = new SslStream(client.GetStream());
        ...
        private void connectToTLSserver(string serverIP, int port)
        {
            IPAddress remoteIP = IPAddress.Parse(serverIP);
            this.client = new TcpClient();
            IPEndPoint IP_End = new IPEndPoint(remoteIP, port);
            try
            {
                ...
                if (client.Connected)
                {
                    ...
                    try
                    {
                        ...
                        Console.WriteLine("Connected to Server: " + IP_End.ToString() + "\n");
                    }
            ...
            else
            {
                ...
                try
                {
                    ...
                    if (bytesRead <= 0)
                    {
                        ...
                        //Console.WriteLine(Thread.CurrentThread.ManagedThreadId + " Connection lost with the client...");
                        //return null;
                    }
                    else
                    {
